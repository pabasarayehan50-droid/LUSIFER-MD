    ...
    try {
        ...
        try {
            ...
            if (!adminStatus.isBotAdmin) {
                await sock.sendMessage(chatId, { 
                    text: '❌ Error: Please make the bot an admin first to use this Lucifer.'
                });
                return;
            }
        ...
        if (error.data === 429) {
            ...
            try {
                await sock.sendMessage(chatId, { 
                    text: '❌ Failed to demote user(s). Make sure the bot is admin and has sufficient permissions Lucifer.'
                });
            } catch (sendError) {
